...
pacman_objects::pacman_objects() {
    orig_pellet_pos.reserve(1000);
    pellet_pos.reserve(1000);
}

pacman_objects::pacman_objects(const vector<vector<int> > &maze) {
    this->maze = maze;
}

void pacman_objects::detect_maze_and_set(const vector<vector<int> > &screen) {
    this->maze = detect_maze(screen);
    this->pellet_pos = detect_pellets(this->maze);
    this->orig_pellet_pos = this->pellet_pos;
}

vector<vector<int> >& pacman_objects::get_maze() {
    return maze;
}

vector<loc>& pacman_objects::get_pellet_pos() {
    return pellet_pos;
}

pair<double, double> pacman_objects::detect_loc(const vector<vector<int> > &screen, int low, int high) {
...
pair<double, double> pacman_objects::detect_pacman_loc(const vector<vector<int> > &screen) {
    return detect_loc(screen, PACMAN_LOW, PACMAN_HIGH);
}

loc pacman_objects::detect_ghost1_loc(const vector<vector<int> > &screen) {
...
loc pacman_objects::detect_ghost2_loc(const vector<vector<int> > &screen) {
...
pair<double, double> pacman_objects::detect_ghost_loc(const vector<vector<int> > &screen, int ghost) {
    return detect_loc(screen, ghost_low[ghost], ghost_high[ghost]);
}

vector<pair<double, double> > pacman_objects::process_screen(const vector<vector<int> > &screen) {
...
pair<bool, bool> pacman_objects::can_move_horizontally(const loc pacman_location)  {
...
pair<bool, bool> pacman_objects::can_move_vertically(const loc pacman_location) {
    int row = round(pacman_location.first);
    int column = round(pacman_location.second);
    int up = 0, down = 0;
    while (up + down < 14) {
...
vector<direction> pacman_objects::get_valid_moves(const loc pacman_location) {
...
vector<loc> pacman_objects::detect_edible_ghosts(const vector<vector<int> > &screen) {
...
Known problem: When a ghost and pellet overlap, pellet gets undetected
               for this step.
*/
void pacman_objects::update_pellet_pos(const vector<vector<int> > &screen) {
